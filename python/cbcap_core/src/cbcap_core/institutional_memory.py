...
class DecisionMemoryRecord(StrictModel):
    """Reviewed institutional decision memory, never chat history.

    Records capture why a planning decision was made, which evidence supported
    it, what was missing, and whether the reasoning can be reused later. The
    record is tenant-private by construction and is not part of the public
    Evidence Gateway contract.
    """
...
class DecisionMemoryProposal(StrictModel):
    ...
    subject_type: str = Field(min_length=1)
    subject_id: str = Field(min_length=1)
    outcome: ExcludeSupersededOutcome
    reason_codes: list[str] = Field(min_length=1)
    rationale: str = Field(min_length=1)
    evidence_entity_ids: list[str] = Field(min_length=1)
    related_entity_ids: list[str] = Field(default_factory=list)
...
ExcludeSupersededOutcome = Literal[
    "accepted",
    "rejected",
    "needs_revision",
    "deferred",
...
def build_decision_memory(
    request: DecisionMemoryWriteRequest,
